...
/*
  follow steps to make it through the puzzle, keep track of accumulator value
  if you reach a step for a second time, break & return acc value
*/

const executeLine = (line, acc) => {
  let newLineOffset = 1;

  let [action, val] = line.split(' ');
  let plusMinus = val[0];
  let multiplier = 1;
  if (plusMinus === '-') {
    multiplier = -1;
  }
  let num = multiplier * parseInt(val.slice(1));
