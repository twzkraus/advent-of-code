...
/*
  i: passport/north pole id fields
  o: number of valid id docs
  c: to be valid, it must contain byr, iyr, eyr,hgt, hcl, ecl, pid
  c: cid is optional

  split up docs by two newline pieces
  check each for the required fields. Valid if all exist
*/

const isValidPassport = (str) => {
  const reqFields = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];
  for (let i = 0; i < reqFields.length; i++) {
    if (str.indexOf(reqFields[i]) < 0) {
      return false;
    }
  }
  return true;
...
const puzzleSolver = (puz) => {
  let count = 0;
  const items = puz.split('\n\n');
  items.forEach(item => {
    if (isValidPassport(item)) {
      count++;
    }
  });
  return count;
...
console.log('part 1 answer', puzzleSolver(puzzle));
// 222 was correct
...
const singleItemValidation = (str, field, func) => {
  let [before, after] = str.split(field + ':');
  return func(after);
}

const isValidatedPassport = (str) => {

  const reqFieldFuncs = {
    'byr': (str) => str.slice(0, 4) >= 1920 && str.slice(0, 4) <= 2002,
    'iyr': (str) => str.slice(0, 4) >= 2010 && str.slice(0, 4) <= 2020,
    'eyr': (str) => str.slice(0, 4) >= 2020 && str.slice(0, 4) <= 2030,
    'hgt': (str) => {
      if (str.includes('cm')) {
        let ht = str.slice(0, 3);
        return ht >= 150 && ht <= 193;
      } else if (str.includes('in')) {
        let ht = str.slice(0, 2);
        return ht >= 59 && ht <= 76;
      } else {
        return false;
      }
    ...
    'hcl': (str) => {
      ...
      for (let i = 1; i < val.length; i++) {
        if (validChars.indexOf(val[i]) < 0) {
          return false;
        }
      }
      return true;
    },
    'ecl': (str) => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].indexOf(str.slice(0, 3)) >= 0,
    'pid': (str) => {
      let val = str.split(/\n|\s/)[0];
      if (val.length !== 9) {
        return false;
  ...
  for (let key in reqFieldFuncs) {
    if (str.indexOf(key) < 0 || !singleItemValidation(str, key, reqFieldFuncs[key])) {
      // console.log(key);
      return false;
    }
  }
...
const puzzleSolver2 = (puz) => {
  let count = 0;
  const items = puz.split('\n\n');
  items.forEach(item => {
    if (isValidatedPassport(item)) {
      count++;
    }
  });
  return count;
...
let test_invalid = `eyr:1972 cid:100
hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926
...
let test_valid = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
console.log(puzzleSolver2(test_invalid), 'should be 0');
console.log(puzzleSolver2(test_valid), 'should be 4');
console.log('part 2 answer', puzzleSolver2(puzzle));
// 69 was incorrect
// 108 was too low
// 140 was correct
