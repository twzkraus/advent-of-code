...
*/
let pwdString = `6-10 p: ctpppjmdpppppp
...
let pwds = pwdString.split('\n');
let validPwds = 0;
pwds.forEach(pwd => {
  let [rule, str] = pwd.split(': ');
  let [nums, targetChar] = rule.split(' ');
  let [num1, num2] = nums.split('-');
  num1 = parseInt(num1);
  num2 = parseInt(num2);
  let charCount = countChar(str, targetChar);
...
const checkSpots = (str, char, p1, p2) => {
  let eitherOr = str[p1 - 1] === char || str[p2 - 1] === char;
  let both = str[p1 - 1] === char && str[p2 - 1] === char;
  return eitherOr && !both;
};
...
pwds.forEach(pwd => {
  ...
  num1 = parseInt(num1);
  num2 = parseInt(num2);
  if (checkSpots(str, targetChar, num1, num2)) {
    validPwds++;
  }
});
