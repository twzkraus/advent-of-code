...
const puzzle = `muted lavender bags contain 5 dull brown bags, 4 pale maroon bags, 2 drab orange bags.
...
const countBags = (puzzle, color) => {
  ...
  const addChild = (parent, child) => {
    ...
    data[parent].canContain.push(child);
  ...
  lines.forEach(line => {
    ...
    rules.forEach(rule => {
      if (rule !== 'no other bags') {
        ...
        addChild(parentColor, childColor);
      }
    });
  });
  ...
  const getAllParents = (color) => {
    ...
    data[color].canBeIn.forEach(parent => {
      parents[parent] = true;
      Object.assign(parents, getAllParents(parent));
    });
    return parents;
  };

  const countAllParents = (color) => {
    const obj = getAllParents(color);
