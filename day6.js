...
/*
  split groups on \n\n
  for each group, compile a list of their yes's
  return the number of yes's
*/

const groups = puzzle.split('\n\n');

const getGroupTotal = (str) => {
  let chars = {};
...
groups.forEach(group => {
  let thisTot = getGroupTotal(group);
  total1 += thisTot;
});
console.log('total where any in the group answered yes', total1);
// 6714 is correct
...
/*
  need to count the number of people in the group and compare the # yes's to the # people
*/

const getPersonCount = (str) => {
  return str.split('\n').length;
};

const getGroupCountForAll = (str) => {
  let chars = {};
  for (let i = 0; i < str.length; i++) {
    if (str[i] !== '\n') {
      if (!chars[str[i]]) {
  ...
  let numForAll = 0;
  let numPeeps = getPersonCount(str);
  for (let key in chars) {
    if (chars[key] === numPeeps) {
      numForAll++;
    }
  }
  return numForAll;
...
let total2 = 0;
groups.forEach(group => {
  let thisTot = getGroupCountForAll(group);
  total2 += thisTot;
});
console.log('total where all in the group answered yes', total2);
//3435 is correct
